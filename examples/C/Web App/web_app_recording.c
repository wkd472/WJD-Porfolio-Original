...
void modifyJS()
{
	system("echo \"function setvalues(){}\" > WebPages/js/md.js");
	system("echo \"function setvalues(){}\" > WebPages/js/schedule.js");
	system("rm WebPages/js/statusbar.js");
	system("rm WebPages/js/recordtime.js");
}
...
/*Variable declaration */
   ...
   struct stat st;
   pthread_t thread_read_line, default_thread_id, thttpdthread;

   if (argc < 2)
   {
      printf ("USAGE : ./web_app WEB_DIRECTORY_PATH [useSDCardFlag]\n");
      exit (0);
   }
   ...
   system("cp sdcard_rewrite /data");
   system("chmod 755 /data/sdcard_rewrite");
   
   system("umount WebPages/playbackfiles");
   system("umount /media/mmcblk0p1/");
      
   system("mkdir -p WebPages/playbackfiles");
	  
   system("mount -t vfat -o defaults,umask=111 /dev/mmcblk0p1 /media/mmcblk0p1/");
   sleep(1);
   system("mount /media/mmcblk0p1/  WebPages/playbackfiles");
   sleep(1);
   

   //system("chmod 644 /media/mmcblk0p1/*.mp4");
   //sleep(1);
   ...
   printf("Starting CRON services\n\n");
   sprintf(cmd,"mkdir -p /var/spool/cron/crontabs");
   system(cmd);
   system("crond");

   pthread_create (&default_thread_id, NULL, (void *) default_thread, NULL);

   pthread_create (&thttpdthread, NULL, thttpdWatch, NULL);
...
default_thread (void *arg)
{
   ...
   system ("mobiapp -f mc2_3s_record.lua");// -f start_record.lua");
}
...
UpdateAllhtml (char *path)
{
  ...
  system("ifconfig eth0 | grep \"Mask\" | awk -F: \'{print $4}\' | awk -F \" \" \'{print $1}\' > /temp/netmask");

   Fps = fopen("/temp/netmask", "rb+");
   if (Fps == NULL)
   {
      perror ("\n\n");
   }
   ...
   system("route | grep \"default\" | awk -F \" \" \'{print $2}\' > /temp/gateway");
   Fps = fopen("/temp/gateway", "rb+");
   if (Fps == NULL)
   {
      perror ("\n\n");
   }
   ...
   if (FileHandle != NULL)
   {
      ...
      fprintf (FileHandle, "<body>\n");
      fprintf (FileHandle,
               "<META HTTP-EQUIV=\"refresh\" CONTENT=\"0;URL=http://%s/operator/alarm.html\">\n",
               ipAddr);
      fprintf (FileHandle, "</body>\n");
      fprintf (FileHandle, "</html>\n");
      fclose (FileHandle);
   ...
   if (FileHandle != NULL)
   {
      ...
      fprintf (FileHandle, "<body>\n");
      fprintf (FileHandle,
               "<META HTTP-EQUIV=\"refresh\" CONTENT=\"0;URL=http://%s/operator/recording.html\">\n",
               ipAddr);
      fprintf (FileHandle, "</body>\n");
      fprintf (FileHandle, "</html>\n");
      fclose (FileHandle);
      ...
      switch (pRecvPacket->pageName)
      {
         case NETWORK_PAGE1:
            ...
            if (pnetwork->changeIP == 1)
            {
               ...
               else
               {
                  ...
                  system ("setIP dhcp");
				 sleep(2);
				 printf("changed ip....now killing spree");
                  pid_1 = GetPIDFromName ("mobiapp");
                  if (pid_1 != 0)
                  {
                     sprintf (cmd, "kill -9 %d", pid_1);
                  ...
                  sleep (2);
				  printf("changed ip..now should reboot");
               }
			   sleep (2);
			   system ("reboot -f");
               exit (0);
		...
		printf("Here in Reboot\n\n\n");
            ...
            else if (psystem->bTimeDate == 1)
            {
               ...
               //printf ("TEMP = %d\n", tmp);
               //sleep (1);
			   //sprintf(str, "export TZ=%s", psystem->chTimeZone);
			   //printf("STR: %s\n", str);
			   //system (str);
               if (psystem->bTimeDateNetwork == 1)
               {
                  memset (str, 0x00, 200);
                  sprintf (str, "ntpdate %s\n", psystem->chIpaddress);
                  system (str);
               }
               else
               {
...
void initFTP()
{
   ...
   system("mount -o nolock /media/mmcblk0p1/  /temp/ftproot/SDCard");
   sleep(1);
   system("umount /temp/ftproot/USBMiniHD");
   system("mount -o nolock /media/sda1/ /temp/ftproot/USBMiniHD");
   sleep(1);
...
void * thttpdWatch(void * arg)
{
	...
	while(1)
	{
		...
		if (pid == 0)
		{
			sprintf (cmd, "./thttpd -d %s -c \"**\" -h %s -u root", cPath, ipAddr);
			system (cmd);
			printf("thttpd restarted");
		}
		sleep (900);
	}
}
